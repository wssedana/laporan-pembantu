...
class lapPeriode extends CI_Controller
{
    ...
    public function cetaXls($periode)
    {
        ...
        $pelanggan = "pelanggan" . substr($periode, 7);
        ...
        foreach ($laporan as $data) {
            ...
            if ($data['tgl_dibaca'] != $tbSebelumnya || $zonaSebelumnya != $data['zona'] || $masaSebelumnya != $data['masa'] || $idDMASebelumnya != $data['id_dma']) {
                $excel->setActiveSheetIndex(0)->setCellValue('D' . $baris, $data['nama_dma']);
                // $jumlahSR = $this->LaporanXls_model->countSRPerDMA($pelanggan, $data['id_dma']);
                // // $excel->setActiveSheetIndex(0)->setCellValue('J' . $baris, $jumlahSR);
                ...
                $excel->getActiveSheet()->getStyle('B' . $baris)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
                $excel->getActiveSheet()->getStyle('C' . $baris)->applyFromArray($style_row);
                $excel->getActiveSheet()->getStyle('C' . $baris)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
                ...
                $excel->getActiveSheet()->getStyle('E' . $baris)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
                $excel->getActiveSheet()->getStyle('F' . $baris)->applyFromArray($style_row);
                $excel->getActiveSheet()->getStyle('F' . $baris)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
                ...
                $excel->getActiveSheet()->getStyle('H' . $baris)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
                ...
                $excel->getActiveSheet()->getStyle('J' . $baris)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
                $baris++;
            }
    ...
    public function cetaXls2($periode)
    {
        ...
        $siang = "";
        $malam = "";
        $periodeBaca = $this->libfunction->format_periode($periode);
        ...
        if (count($laporan) == 0) {
            ...
            $excel->getActiveSheet()->mergeCells('A' . $baris . ':B' . $baris); // Set Merge Cell pada kolom A1 sampai E1
            $excel->setActiveSheetIndex(0)->setCellValue('C' . $baris, $periodeBaca);
        ...
        foreach ($laporan as $data) {
            if ($data['tgl_dibaca'] != $tbSebelumnya || $zonaSebelumnya != $data['zona'] || $masaSebelumnya != $data['masa']) {
                if ($mulai > 0) {
                    ...
                    $excel->setActiveSheetIndex(0)->setCellValue('D'  . $baris, "Menyetujui"); // Set kolom A1 dengan tulisan "DATA SISWA"    
                    $excel->getActiveSheet()->mergeCells('D' . $baris . ':H' . $baris); // Set Merge Cell pada kolom A BARIS sampai A BARIS+2
                    $excel->getActiveSheet()->getStyle('D' . $baris)->applyFromArray($ttd);

                    $excel->setActiveSheetIndex(0)->setCellValue('I'  . $baris, "Dibuat Oleh"); // Set kolom A1 dengan tulisan "DATA SISWA"    
                    $excel->getActiveSheet()->mergeCells('I' . $baris . ':K' . $baris); // Set Merge Cell pada kolom A BARIS sampai A BARIS+2
                    $excel->getActiveSheet()->getStyle('I' . $baris)->applyFromArray($ttd);
                    $baris = $baris + 1;

                    $excel->setActiveSheetIndex(0)->setCellValue('A'  . $baris, "Kepala Cabang " . $kecamatan = ucwords($kecamatanSebelumnya)); // Set kolom A1 dengan tulisan "DATA SISWA"    
                    $excel->getActiveSheet()->mergeCells('A' . $baris . ':C' . $baris); // Set Merge Cell pada kolom A BARIS sampai A BARIS+2
                    $excel->getActiveSheet()->getStyle('A' . $baris)->applyFromArray($ttd);

                    $excel->setActiveSheetIndex(0)->setCellValue('D'  . $baris, "Kasi.Teknik Cabang " . $kecamatan = ucwords($kecamatanSebelumnya)); // Set kolom A1 dengan tulisan "DATA SISWA"    
                    $excel->getActiveSheet()->mergeCells('D' . $baris . ':H' . $baris); // Set Merge Cell pada kolom A BARIS sampai A BARIS+2
                    $excel->getActiveSheet()->getStyle('D' . $baris)->applyFromArray($ttd);

                    $excel->setActiveSheetIndex(0)->setCellValue('I'  . $baris, "Petugas Monitoring"); // Set kolom A1 dengan tulisan "DATA SISWA"    
                    $excel->getActiveSheet()->mergeCells('I' . $baris . ':K' . $baris); // Set Merge Cell pada kolom A BARIS sampai A BARIS+2
                    $excel->getActiveSheet()->getStyle('I' . $baris)->applyFromArray($ttd);
                    $baris = $baris + 4;
                    $ambilTTD = $this->LaporanXls_model->ambilTTD($idAreaKerjaSebelumnya, 'KC')->row_array();
                    $excel->setActiveSheetIndex(0)->setCellValue('A'  . $baris, $ambilTTD['nama']); // Set kolom A1 dengan tulisan "DATA SISWA"    
                    $excel->getActiveSheet()->mergeCells('A' . $baris . ':C' . $baris); // Set Merge Cell pada kolom A BARIS sampai A BARIS+2
                    $excel->getActiveSheet()->getStyle('A' . $baris)->applyFromArray($ttdNama);

                    $ambilTTD = $this->LaporanXls_model->ambilTTD($idAreaKerjaSebelumnya, 'KS')->row_array();
                    $excel->setActiveSheetIndex(0)->setCellValue('D'  . $baris, $ambilTTD['nama']); // Set kolom A1 dengan tulisan "DATA SISWA"    
                    $excel->getActiveSheet()->mergeCells('D' . $baris . ':H' . $baris); // Set Merge Cell pada kolom A BARIS sampai A BARIS+2
                    $excel->getActiveSheet()->getStyle('D' . $baris)->applyFromArray($ttdNama);

                    $ambilTTD = $this->LaporanXls_model->ambilTTD($idAreaKerjaSebelumnya, 'STF', $nikSebelumnya)->row_array();
                    $excel->setActiveSheetIndex(0)->setCellValue('I'  . $baris, $ambilTTD['nama']); // Set kolom A1 dengan tulisan "DATA SISWA"    
                    $excel->getActiveSheet()->mergeCells('I' . $baris . ':K' . $baris); // Set Merge Cell pada kolom A BARIS sampai A BARIS+2
                    $excel->getActiveSheet()->getStyle('I' . $baris)->applyFromArray($ttdNama);
                    $baris = $baris + 1;
                    ...
                    $ambilTTD = $this->LaporanXls_model->ambilTTD($idAreaKerjaSebelumnya, 'STF', $nikSebelumnya)->row_array();
                    $excel->setActiveSheetIndex(0)->setCellValue('I'  . $baris, "NIK : " . $ambilTTD['nik']); // Set kolom A1 dengan tulisan "DATA SISWA"    
                    $excel->getActiveSheet()->mergeCells('I' . $baris . ':K' . $baris); // Set Merge Cell pada kolom A BARIS sampai A BARIS+2
                    $excel->getActiveSheet()->getStyle('I' . $baris)->applyFromArray($ttd);
                    $baris = $baris + 1;
        ...
        $excel->getActiveSheet()->getColumnDimension('B')->setWidth(10); // Set Autosize width kolom B
        $excel->getActiveSheet()->getColumnDimension('C')->setWidth(10); // Set Autosize width kolom C
        $excel->getActiveSheet()->getColumnDimension('D')->setWidth(30); // Set Autosize width kolom D
        $excel->getActiveSheet()->getColumnDimension('E')->setAutoSize(true); // Set Autosize width kolom E
        $excel->getActiveSheet()->getColumnDimension('F')->setAutoSize(true); // Set Autosize width kolom F
        $excel->getActiveSheet()->getColumnDimension('G')->setWidth(7); // Set Autosize width kolom G
        ...
        $excel->getActiveSheet()->getColumnDimension('J')->setWidth(8); // Set Autosize width kolom J
        $excel->getActiveSheet()->getColumnDimension('K')->setWidth(12); // Set Autosize width kolom J
        ...
        // Proses file excel
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment; filename="LAPORAN PENGECEKAN TEKANAN MANOMETER PERIODE ' . $periodeBaca . '.xlsx"'); // Set nama file excel nya
        header('Cache-Control: max-age=0');
        $write = PHPExcel_IOFactory::createWriter($excel, 'Excel2007');
        $write->save('php://output');
        //akhir export excel    
    ...
    public function rekapXls($periode)
    {
        ...
        $ttdNama = array(
            'font' => array('size' => 11, 'underline' => true), // Set font nya jadi bold
            'alignment' => array(
                'horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER, // Set text jadi ditengah secara horizontal (center)
                'vertical' => PHPExcel_Style_Alignment::VERTICAL_CENTER // Set text jadi di tengah secara vertical (middle)
            )
        ...
        foreach ($dataManoMeter as $data) {
            ...
            if ($zonaSebelumnya != $data['zona']) {
                if ($mulai > 0) {
                    $baris = $baris + 2;
                    $excel->setActiveSheetIndex(0)->setCellValue('B'  . $baris, "Menyetujui"); // Set kolom A1 dengan tulisan "DATA SISWA"    
                    $excel->getActiveSheet()->mergeCells('B' . $baris . ':D' . $baris); // Set Merge Cell pada kolom A BARIS sampai A BARIS+2
                    $excel->getActiveSheet()->getStyle('B' . $baris)->applyFromArray($ttd);

                    $excel->setActiveSheetIndex(0)->setCellValue('G'  . $baris, "Mengetahui"); // Set kolom A1 dengan tulisan "DATA SISWA"    
                    $excel->getActiveSheet()->mergeCells('G' . $baris . ':J' . $baris); // Set Merge Cell pada kolom A BARIS sampai A BARIS+2
                    $excel->getActiveSheet()->getStyle('G' . $baris)->applyFromArray($ttd);

                    $excel->setActiveSheetIndex(0)->setCellValue('N'  . $baris, "Dibuat Oleh"); // Set kolom A1 dengan tulisan "DATA SISWA"    
                    $excel->getActiveSheet()->mergeCells('N' . $baris . ':P' . $baris); // Set Merge Cell pada kolom A BARIS sampai A BARIS+2
                    $excel->getActiveSheet()->getStyle('N' . $baris)->applyFromArray($ttd);
                    $baris = $baris + 1;

                    $excel->setActiveSheetIndex(0)->setCellValue('B'  . $baris, "Kepala Cabang " . ucwords($kecamatanSebelumnya)); // Set kolom A1 dengan tulisan "DATA SISWA"    
                    $excel->getActiveSheet()->mergeCells('B' . $baris . ':D' . $baris); // Set Merge Cell pada kolom A BARIS sampai A BARIS+2
                    $excel->getActiveSheet()->getStyle('B' . $baris)->applyFromArray($ttd);

                    $excel->setActiveSheetIndex(0)->setCellValue('G'  . $baris, "Kasi.Teknik Cabang " . ucwords($kecamatanSebelumnya)); // Set kolom A1 dengan tulisan "DATA SISWA"    
                    $excel->getActiveSheet()->mergeCells('G' . $baris . ':J' . $baris); // Set Merge Cell pada kolom A BARIS sampai A BARIS+2
                    $excel->getActiveSheet()->getStyle('G' . $baris)->applyFromArray($ttd);

                    $excel->setActiveSheetIndex(0)->setCellValue('N'  . $baris, "Petugas Monitoring"); // Set kolom A1 dengan tulisan "DATA SISWA"    
                    $excel->getActiveSheet()->mergeCells('N' . $baris . ':P' . $baris); // Set Merge Cell pada kolom A BARIS sampai A BARIS+2
                    $excel->getActiveSheet()->getStyle('N' . $baris)->applyFromArray($ttd);
                    $baris = $baris + 4;
                    $ambilTTD = $this->LaporanXls_model->ambilTTD($idAreaKerjaSebelumnya, 'KC')->row_array();
                    $excel->setActiveSheetIndex(0)->setCellValue('B'  . $baris, $ambilTTD['nama']); // Set kolom A1 dengan tulisan "DATA SISWA"    
                    $excel->getActiveSheet()->mergeCells('B' . $baris . ':D' . $baris); // Set Merge Cell pada kolom A BARIS sampai A BARIS+2
                    $excel->getActiveSheet()->getStyle('B' . $baris)->applyFromArray($ttdNama);

                    $ambilTTD = $this->LaporanXls_model->ambilTTD($idAreaKerjaSebelumnya, 'KS')->row_array();
                    $excel->setActiveSheetIndex(0)->setCellValue('G'  . $baris, $ambilTTD['nama']); // Set kolom A1 dengan tulisan "DATA SISWA"    
                    $excel->getActiveSheet()->mergeCells('G' . $baris . ':J' . $baris); // Set Merge Cell pada kolom A BARIS sampai A BARIS+2
                    $excel->getActiveSheet()->getStyle('G' . $baris)->applyFromArray($ttdNama);

                    $ambilTTD = $this->LaporanXls_model->ambilTTD($idAreaKerjaSebelumnya, 'STF', $nikSebelumnya)->row_array();
                    $excel->setActiveSheetIndex(0)->setCellValue('N'  . $baris, $ambilTTD['nama']); // Set kolom A1 dengan tulisan "DATA SISWA"    
                    $excel->getActiveSheet()->mergeCells('N' . $baris . ':P' . $baris); // Set Merge Cell pada kolom A BARIS sampai A BARIS+2
                    $excel->getActiveSheet()->getStyle('N' . $baris)->applyFromArray($ttdNama);
                    $baris = $baris + 1;

                    $ambilTTD = $this->LaporanXls_model->ambilTTD($idAreaKerjaSebelumnya, 'KC')->row_array();
                    $excel->setActiveSheetIndex(0)->setCellValue('B'  . $baris, $ambilTTD['pangkat']); // Set kolom A1 dengan tulisan "DATA SISWA"    
                    $excel->getActiveSheet()->mergeCells('B' . $baris . ':D' . $baris); // Set Merge Cell pada kolom A BARIS sampai A BARIS+2
                    $excel->getActiveSheet()->getStyle('B' . $baris)->applyFromArray($ttd);

                    $ambilTTD = $this->LaporanXls_model->ambilTTD($idAreaKerjaSebelumnya, 'KS')->row_array();
                    $excel->setActiveSheetIndex(0)->setCellValue('G'  . $baris, $ambilTTD['pangkat']); // Set kolom A1 dengan tulisan "DATA SISWA"    
                    $excel->getActiveSheet()->mergeCells('G' . $baris . ':J' . $baris); // Set Merge Cell pada kolom A BARIS sampai A BARIS+2
                    $excel->getActiveSheet()->getStyle('G' . $baris)->applyFromArray($ttd);

                    $ambilTTD = $this->LaporanXls_model->ambilTTD($idAreaKerjaSebelumnya, 'STF', $nikSebelumnya)->row_array();
                    $excel->setActiveSheetIndex(0)->setCellValue('N'  . $baris, "NIK : " . $ambilTTD['nik']); // Set kolom A1 dengan tulisan "DATA SISWA"    
                    $excel->getActiveSheet()->mergeCells('N' . $baris . ':P' . $baris); // Set Merge Cell pada kolom A BARIS sampai A BARIS+2
                    $excel->getActiveSheet()->getStyle('N' . $baris)->applyFromArray($ttd);
                    $baris = $baris + 1;

                    $ambilTTD = $this->LaporanXls_model->ambilTTD($idAreaKerjaSebelumnya, 'KC')->row_array();
                    $excel->setActiveSheetIndex(0)->setCellValue('B'  . $baris, "NIK : " . $ambilTTD['nik']); // Set kolom A1 dengan tulisan "DATA SISWA"    
                    $excel->getActiveSheet()->mergeCells('B' . $baris . ':D' . $baris); // Set Merge Cell pada kolom A BARIS sampai A BARIS+2
                    $excel->getActiveSheet()->getStyle('B' . $baris)->applyFromArray($ttd);
                ...
                if (count($getSore) > 0) {
                    ...
                    $columnSoreAkhir = $columnPagiAkhir;
                }
                ...
                if (count($getSiang) > 0) {
                    ...
                    $columnSiangAkhir = $columnSoreAkhir;
                }
                ...
                if (count($getMalam) > 0) {
                    ...
                    $columnMalamAkhir = $columnSiangAkhir;
                }
                ...
                $excel->getActiveSheet()->getStyle(chr(ord($column)) . $baris8)->getAlignment()->setWrapText(true);
                // $excel->getActiveSheet()->mergeCells(chr(ord($column)) . $baris8 . ':' . chr(ord($column)) . $baris8 = $baris8 + 2);
                $excel->getActiveSheet()->getStyle(chr(ord($column)) . $baris8)->applyFromArray($style_col);
                $excel->getActiveSheet()->getStyle(chr(ord($column)) . $baris8 = $baris8 + 1)->applyFromArray($style_col);
                $excel->getActiveSheet()->getStyle(chr(ord($column)) . $baris8 = $baris8 + 1)->applyFromArray($style_col);
            ...
            if ($zonaSebelumnya != $data['zona'] || $idDMASebelumnya != $data['id_dma']) {
                $excel->setActiveSheetIndex(0)->setCellValue('D' . $baris, $data['nama_dma']);
                $columnDMA = "A";
                for ($merge = 0; $merge < $getSemuaMasa + $i + 7; $merge++) {
                    $excel->getActiveSheet(0)->getStyle($columnDMA . $baris)->applyFromArray($style_row);
                    $columnDMA = chr(ord($columnDMA) + 1);
                }
            ...
            if (count($getPagi) > 0) {
                foreach ($getPagi as $row) {
                    ...
                    if (count($presure) > 0) {
                        ...
                        $tekananBenar = implode(' , ', $tekanan);
                    }
                    $excel->setActiveSheetIndex(0)->setCellValue($column = chr(ord($column) + 1) . $baris, $tekananBenar);
            ...
            if (count($dataManoMeter) == $a) {
                ...
                $excel->setActiveSheetIndex(0)->setCellValue('B'  . $baris, "Kepala Cabang " . ucwords($data['kecamatan'])); // Set kolom A1 dengan tulisan "DATA SISWA"    
                $excel->getActiveSheet()->mergeCells('B' . $baris . ':D' . $baris); // Set Merge Cell pada kolom A BARIS sampai A BARIS+2
                $excel->getActiveSheet()->getStyle('B' . $baris)->applyFromArray($ttd);

                $excel->setActiveSheetIndex(0)->setCellValue('G'  . $baris, "Kasi.Teknik Cabang " . ucwords($data['kecamatan'])); // Set kolom A1 dengan tulisan "DATA SISWA"    
                $excel->getActiveSheet()->mergeCells('G' . $baris . ':J' . $baris); // Set Merge Cell pada kolom A BARIS sampai A BARIS+2
                $excel->getActiveSheet()->getStyle('G' . $baris)->applyFromArray($ttd);
        ...
        // Proses file excel
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header('Content-Disposition: attachment; filename="REKAPITULASI PENGECEKAN TEKANAN MANOMETER PERIODE ' . $periodeBaca . '.xlsx"'); // Set nama file excel nya
        header('Cache-Control: max-age=0');
        $write = PHPExcel_IOFactory::createWriter($excel, 'Excel2007');
        $write->save('php://output');
        //akhir export excel    
