        ...
        <div class="row">
            <div class="col">
                <div class="input-group">
                    ...
                    <input type="text" name="pelanggan" id="pelanggan" class="form-control" value="<?= $result['jml_pelanggan']; ?>" disabled>
                </div>
            </div>
        </div>
        <br />
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('#delete').click(function() {
            var id_zona = $('#id_zona').val();
            if (confirm("Yakin hapus zona? \n zona yang dihapus tidak dapat dikembalikan lagi !")) {
                $.ajax({
                    url: '<?= base_url('masterData/delete') ?>',
                    method: 'post',
                    data: {
                        id_zona: id_zona
                    },
                    success: function(data) {
                        if (data == "success") {
                            alert("Success delete data zona");
                        } else {
                            alert("Gagal delete data zona");
                        }
                        window.location.replace('<?= base_url('masterData/zona') ?>');
                    }
                });
